...
use crate::mm::MAX_USERSPACE_VADDR;
...
pub trait VmIo: Send + Sync {
    ...
    ///
    /// [`read`]: VmIo::read
    fn read_bytes(&self, offset: usize, buf: &mut [u8]) -> Result<(), crate::error::Error> {
    ...
    fn read_val<T: Pod>(&self, offset: usize) -> Result<T, crate::error::Error> {
    ...
    ///
    /// [`write`]: VmIo::write
    fn write_bytes(&self, offset: usize, buf: &[u8]) -> Result<(), crate::error::Error> {
        let mut reader = VmReader::from(buf).to_fallible();
        self.write(offset, &mut reader)
    }

    /// Writes a value of a specified type at a specified offset.
    fn write_val<T: Pod>(&self, offset: usize, new_val: &T) -> Result<(), crate::error::Error> {
    ...
    ///
    /// [`write`]: VmIo::write
    fn write_slice<T: Pod>(&self, offset: usize, slice: &[T]) -> Result<(), crate::error::Error> {
        let len_in_bytes = core::mem::size_of_val(slice);
...
pub trait VmIoOnce {
    ...
    /// [`VmReader::read_once`].
    fn read_once<T: PodOnce>(&self, offset: usize) -> Result<T, crate::error::Error>;
    ...
    /// Except that the offset is specified explicitly, the semantics of this method is the same as
    /// [`VmWriter::write_once`].
    fn write_once<T: PodOnce>(&self, offset: usize, new_val: &T)
    -> Result<(), crate::error::Error>;
}
...
impl<'a> VmReader<'a, Infallible> {
    ...
    pub unsafe fn from_kernel_space(ptr: *const u8, len: usize) -> Self {
        ...
        Self { cursor: ptr, end: ptr.add(len), phantom: PhantomData }
    }
    ...
    pub fn read_val<T: Pod>(&mut self) -> Result<T, crate::error::Error> {
    ...
    pub fn read_once<T: PodOnce>(&mut self) -> Result<T, crate::error::Error> {
        if self.remain() < core::mem::size_of::<T>() {
            return Err(Error::InvalidArgs);
        }
...
impl VmReader<'_, Fallible> {
    ...
    pub fn read_val<T: Pod>(&mut self) -> Result<T, crate::error::Error> {
        ...
        let mut val = T::new_uninit();
        let mut writer = VmWriter::from(val.as_bytes_mut());
        self.read_fallible(&mut writer).map_err(|(err, copied_len)| {
    ...
    /// and the current reader's cursor remains pointing to
    /// the original starting position.
    pub fn collect(&mut self) -> Result<Vec<u8>, crate::error::Error> {
        let mut buf = vec![0u8; self.remain()];
        self.read_fallible(&mut buf.as_mut_slice().into()).map_err(|(err, copied_len)| {
            // SAFETY: The `copied_len` is the number of bytes read so far.
            // So the `cursor` can be moved back to the original position.
            unsafe {
                self.cursor = self.cursor.sub(copied_len);
...
impl<'a> VmWriter<'a, Infallible> {
    ...
    /// this method will return `Err`.
    pub fn write_val<T: Pod>(&mut self, new_val: &T) -> Result<(), crate::error::Error> {
    ...
    /// requirements of type `T`.
    pub fn write_once<T: PodOnce>(&mut self, new_val: &T) -> Result<(), crate::error::Error> {
        if self.avail() < core::mem::size_of::<T>() {
            return Err(Error::InvalidArgs);
        }
...
impl VmWriter<'_, Fallible> {
    ...
    pub fn write_val<T: Pod>(&mut self, new_val: &T) -> Result<(), crate::error::Error> {
        ...
        let mut reader = VmReader::from(new_val.as_bytes());
        self.write_fallible(&mut reader).map_err(|(err, copied_len)| {
