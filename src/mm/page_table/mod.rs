...
use super::page::meta::MapTrackingStatus;
use super::page_prop::PageProperty;
use super::{Paddr, PagingConstsTrait, PagingLevel, Vaddr, nr_subpage_per_huge, page_size};
use crate::Pod;
...
where
    [(); C::NR_LEVELS as usize]:,
{
    /// Create self from root physical address.
    pub unsafe fn from_root_paddr(paddr: Paddr, level: PagingLevel) -> Self {
        Self { root: RawPageTableNode::from_raw_parts(paddr, level), _phantom: PhantomData }
    }
    ...
    pub unsafe fn shallow_copy(&self) -> Self {
        PageTable { root: self.root.clone_shallow(), _phantom: PhantomData }
    }
}
